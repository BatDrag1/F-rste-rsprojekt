...
def get_n_best_correspodences(scan_descriptors,ref_descriptors,n,threshold,method):
    ...
    """
    non_empty_descriptors = np.any(scan_descriptors, axis=1).nonzero()[0]
    print(len(non_empty_descriptors))
    non_empty_ref_descriptors = np.any(ref_descriptors, axis=1).nonzero()[0]
    print(len(non_empty_ref_descriptors))
    ...
    k = 0
    while continues:
        #print("k: "+str(k)+ " scan point: "+ str(data_set[k][0])+" brøk: "+ str(data_set[k-1][2] / data_set[k][2]))
        if k == nr_of_point_in_scan - 1:
            continues = False
        elif len(trimmed_data_set) < 150:
            trimmed_data_set.append(data_set[k])
        elif (data_set[k-1][2] / data_set[k][2] > threshold):
            #print(data_set[k-1][2] / data_set[k][2])
            trimmed_data_set.append(data_set[k])
        else:
            continues = False
        k = k + 1
